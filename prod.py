    ...
    def choose_file(self):
        ...
        if filename:
            ...
            for i in range(4, ws.max_row + 1):                                                              # перебираем строки исх таблицы
                ...
                nom_bu = str(ws.cell(row=i, column=(int(rb)+1)).value)                                                   # получаем номер БУ 
                ...
                if('http' in str(adressURL)):    # возможно  это даже ссылка
                    ...
                    if (send.status_code == 200) and (len(send.text) < 10000000) : # если код ответа по загрузке успешен и размер не больше 10 Мб продолжаем обработку
                        ...
                        if begin_of_text.startswith('%PDF-'):     
                            ...
                            name_of_JPG_file ='c:/bu/tmp/' +str(nom_bu)+'.jpg'    # jpg
#                            with open(name_of_JPG_file,"wb")as code:
 #                               code.write(send.content)
                            out=open(name_of_JPG_file,"wb")
                            out.write(send.content)
                            out.close()
                                #записываем результат  в папку tmp
                            # распознование jpg файла будет тут
                            img = Image.open(name_of_JPG_file)
                            text = pytesseract.image_to_string(img, lang='rus').strip()
                            number_of_pages=1
                            row_links.append(number_of_pages) #
                            new_text = text.replace('\n\n\n', '\n')
